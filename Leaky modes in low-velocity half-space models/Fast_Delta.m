function dd = Fast_Delta(k,om,thk,dns,cvp,cvs,flag)
...
%   rk(real(rk)<0)= -rk(real(rk)<0);  sk(real(sk)>0)= -sk(real(sk)>0);
   ...
   if nargin == 6
       ...
       if real(sk(end))<0
           sk(end)=-sk(end); %% normal modes
       end
   elseif nargin == 7
       if real(sk(end))>0
           sk(end)=-sk(end); %% leaky modes
       end
